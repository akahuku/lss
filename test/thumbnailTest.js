...
describe('thumbnailUtils', () => {
	beforeEach(() => {
		thumbnailUtils.initChildNames();
		thumbnailUtils.clearThumbnailers();
	...
	it('type 3,1 alternative: content thumbnail generated by imagemagick directly', () => {
		const target = path.join(__dirname, 'lenna.jpg');

		thumbnailUtils.clearSixelCache(target);
		thumbnailUtils.clearPngCache(target);
		// this makes the initThumbnailerInfo() function build fallback thumbnailers
		thumbnailUtils.childNames['gdk-pixbuf-thumbnailer'] = 'not-exist-executable';
